  ...
  <u:${action} xmlns:u="${urn}">
   ${body}
    ...
    static async turnOnAppleTv(appleTvConfig, log) {
        ...
        const strategies = [
            { label: 'turn_on (companion)', args: [...buildArgs('companion', 49152), 'turn_on'] },
            { label: 'turn_on (airplay)', args: [...buildArgs('airplay', 7000), 'turn_on'] },
            { label: 'launch_app (companion)', args: [...buildArgs('companion', 49152), 'launch_app=com.apple.TVWatchList'] },
            { label: 'home_hold (companion)', args: [...buildArgs('companion', 49152), 'home_hold'] },
        ];
    ...
    static async pairStart(identifier, address, protocol, log) {
        ...
        const defaultPorts = { airplay: 7000, companion: 49152 };
        const args = ['-m'];
