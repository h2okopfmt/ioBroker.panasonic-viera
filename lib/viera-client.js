   ...
   ${body}
    ...
    static sendWakeOnLan(mac, ip) {
        return new Promise((resolve, reject) => {
            ...
            // Calculate subnet broadcast from IP (e.g. 192.168.1.122 -> 192.168.1.255)
            ...
            // Send to multiple addresses for maximum compatibility (especially Docker)
            socket.bind(() => {
                socket.setBroadcast(true);
                let sent = 0;
                const targets = [broadcastAddr, '255.255.255.255'];
                if (ip) targets.push(ip);

                for (const target of targets) {
                    socket.send(magicPacket, 0, magicPacket.length, 9, target, () => {
                        sent++;
