  ...
  <u:${action} xmlns:u="${urn}">
   ${body}
    ...
    static turnOnAppleTv(appleTvConfig, log) {
        const atvscriptPath = VieraClient._findBinary('atvscript');
        const args = [];

        if (appleTvConfig.identifier) {
            args.push('--id', appleTvConfig.identifier);
        }
        if (appleTvConfig.address) {
            args.push('-s', appleTvConfig.address);
        }
        ...
        if (log) log.debug(`Executing: ${atvscriptPath} ${args.map(a => a.length > 20 ? a.substring(0, 20) + '...' : a).join(' ')}`);

        return new Promise((resolve, reject) => {
            execFile(atvscriptPath, args, { timeout: 15000 }, (error, stdout, stderr) => {
                if (error) {
                    reject(new Error(`atvscript turn_on failed: ${error.message}${stderr ? '. ' + stderr : ''}`));
                    return;
                }
                try {
                    ...
                    resolve({ result: 'success', raw: stdout.trim() });
                }
            });
        });
    }
